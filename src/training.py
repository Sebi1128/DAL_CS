...
def epoch_run(model, active_dataset, optimizer, run_no, model_writer, cfg):
    ...
    for epoch_no in pbar:
        c_train_loss, r_train_loss, s_train_loss = train_epoch(model, active_dataset, optimizer,
                                                 batch_size=cfg.batch_size, device=cfg.device)
        c_valid_loss, r_valid_loss = validate_epoch(model, active_dataset,
                                                    batch_size=cfg.batch_size, device=cfg.device)

        info_text = f"{run_no+1}|C|R|S Train: {c_train_loss:.5f}|{r_train_loss:.5f}|{s_train_loss:.5f}"
        info_text += f"  Valid: {c_valid_loss:.5f}|{r_valid_loss:.5f}"
        pbar.set_description(info_text, refresh=True)

        wandb.log({"r_train_loss": r_train_loss, "c_train_loss": c_train_loss, "s_train_loss": s_train_loss})
...
def train_epoch(model, active_data, optimizer, batch_size, device):
    ...
    for is_labeled in iter_schedule[:n_epochs]:
        ...
        if is_labeled:
            ...
            inp = (r_labeled, r_unlabeled)
            s_loss = model.sampler(inp)
            loss += s_loss
    ...
    mean_s_loss = torch.mean(torch.tensor(s_losses))

    return mean_c_loss, mean_r_loss, mean_s_loss
